...
often calling the `tskit` C API.
However, the overhead can be limiting for repeated calls between R and Python,
for example, tree sequence recording in an R session or package or
calling Python from within an R loop.
...
The authors are open to expanding the scope of `RcppTskit` depending on user demand
and engagement.
...
after we describe the data and class model.
...
the `reticulate` option to call the `tskit` Python API.
