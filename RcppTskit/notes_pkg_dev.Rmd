...
the low-level `tc_xptr_to_ts_xptr(..., options)` wrapper.
...
- `tc_xptr_to_ts_xptr()` calls `tsk_treeseq_init(...)` without `TSK_TAKE_OWNERSHIP`.
...
tree_sequence = function() {
  ...
  ts_xptr <- tc_xptr_to_ts_xptr(self$pointer, options = 0L)
...
- `tc_xptr_build_index()` + `TableCollection$build_index()`
- `tc_xptr_drop_index()` + `TableCollection$drop_index()`
...
1. Add `TableCollection$build_index()` / `$drop_index()` (+ `tc_xptr_*` wrappers).
...
5. Keep `tc_xptr_to_ts_xptr(..., options)` as low-level C-like API (do not remove).
